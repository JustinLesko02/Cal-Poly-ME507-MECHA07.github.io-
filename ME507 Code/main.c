...
/**
 * @brief The enum of valid states for the main FSM
 */
typedef enum {
	S0_INTERFACE,
	S1_ADC,
...
main_state_t mainState = S0_INTERFACE; /**< the next state of the main FSM to run*/
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
    while (1)
    {
    	...
    	if (mainState == S0_INTERFACE) { // state 0 initializes the wifi interface
    		if (prevMainState != mainState) {

    			 print_msg("MAIN STATE: 0\r\n");
    			 prevMainState = mainState;
    		}
    		if (start_interface() == 1) { // start interface is the main fsm to start wifi and returns 1 when complete
    			mainState = S1_ADC;
    		}
    	} else if (mainState == S1_ADC) { // state 1 initializes the ADC
    		if (prevMainState != mainState) {

    			print_msg("MAIN STATE: 1\r\n");
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_10|GPIO_PIN_14|GPIO_PIN_15|GPIO_PIN_0
                          |GPIO_PIN_2|GPIO_PIN_5, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_11|GPIO_PIN_13|GPIO_PIN_1|GPIO_PIN_3, GPIO_PIN_SET);
...
/* USER CODE BEGIN 4 */
/**
 * @brief Interrupt callback for data received via the uart connection to the wifi module.  Forwards to wifi_interrupt
 * @param huart: pointer to the wifi uart handler
 * @retval: None
 */
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	wifi_interrupt(huart);
}
